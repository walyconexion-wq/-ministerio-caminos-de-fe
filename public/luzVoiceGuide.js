...
(function() {
  ...
  function injectPlayerUI() {
    ...
    widget.innerHTML = `
      <!-- ESTADO 1: BOTÓN DE ACTIVACIÓN COMPACTO (Inicial) -->
      <button id="btn-activate-voice-guide" type="button" class="group flex items-center gap-2.5 px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl bg-[#0b0f19]/90 hover:bg-[#131b2e] border border-amber-500/40 shadow-lg shadow-black/70 backdrop-blur-md transition-all transform hover:scale-105 active:scale-95 text-left cursor-pointer">
  ...
  function bindUIEvents() {
    ...
    if (btnActivate) {
      btnActivate.addEventListener('click', () => {
        ...
        updateCurrentLabel('Bienvenida');
        speakScript(SECTIONS_SCRIPTS['hero'].text);
      });
    }
